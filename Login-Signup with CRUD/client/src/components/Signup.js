...
const Signup = () => {
  ...
  useEffect(() => {
    ...
    if (storedEmail && storedEmail !== paramEmail) {
      ...
      setInputData(prev => ({ ...prev, email: '' })); // Clear the email input field if there's a mismatch
    } else {
      setInputData(prev => ({ ...prev, email: paramEmail })); // Set the email if it matches or if there's no stored email
