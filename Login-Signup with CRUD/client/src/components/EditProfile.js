...
const EditProfile = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      setImagePreview(`http://localhost:5001/${image}`);
      //fetchCurrentPassword(token);
    }
  }, [image,navigate]);

//   const fetchCurrentPassword = async (token) => {
//     try {
//       const response = await fetch('http://localhost:5001/api/get-current-password', {
//         method: 'GET',
//         headers: {
...
//       } else {
//         toast('Error fetching current password');
//       }
//     } catch (error) {
//       toast('Server error');
//     }
